...
from model.networks.gat_networks.gat import GraphAttentionNetworks
from model.graph import _init_weights, make_mlp
import numpy as np
...
class Sg2ScVAEModel(nn.Module):
    ...
    def __init__(
        ...
        gat_dim = embedding_dim
        gat_hidden_dim = gat_dim * gat_heads
        ...
        self.mean_var = make_mlp(
            [embedding_dim * 2, gat_hidden_dim, embedding_dim * 2],
        ...
        self.gat_net_ec = None
        self.gat_net_dc = None

        gat_kwargs_ec = {
            "input_dim_obj": gat_dim * 2,
            "input_dim_pred": gat_dim * 2,
        ...
        gat_kwargs_dc = {
            "input_dim_obj": gat_dim + dec_unit_addition_dim,
            "input_dim_pred": gat_dim,
            "hidden_dim": gat_hidden_dim,
            "num_layers": gat_num_layers,
            "num_heads": gat_heads,
            "gat_type": gat_type,
        ...
        gat_kwargs_manipulation = {
            "input_dim_obj": embedding_dim * 3 + dec_unit_addition_dim,
        ...
        if self.decoder_cat:
            gat_kwargs_dc["input_dim_obj"] = gat_dim * 2 + dec_unit_addition_dim
            gat_kwargs_dc["input_dim_pred"] = gat_dim * 2

        self.gat_net_ec = GraphAttentionNetworks(**gat_kwargs_ec)
        self.gat_net_dc = GraphAttentionNetworks(**gat_kwargs_dc)
        self.gat_net_manipulation = GraphAttentionNetworks(**gat_kwargs_manipulation)

        net_layers = [
            gat_dim * 2 + dec_unit_addition_dim + dec_unit_addition_box_dim,
            gat_hidden_dim,
            input_dim,
        ]
    ...
    def encoder(
        ...
        if self.gat_net_ec is not None:
            obj_vecs, pred_vecs = self.gat_net_ec(obj_vecs, pred_vecs, edges)
    ...
    def manipulate(
        ...
        man_z, _ = self.gat_net_manipulation(man_z, pred_vecs, edges)
    ...
    def decoder(
        ...
        if self.decoder_cat:
            ...
            obj_vecs_, pred_vecs = self.gat_net_dc(obj_vecs_, pred_vecs, edges)

        # concatenate noise after gnn
        else:
            obj_vecs_, pred_vecs = self.gat_net_dc(obj_vecs_, pred_vecs, edges)
            obj_vecs_ = torch.cat([obj_vecs_, z], dim=1)
