...
class GraphAttentionNetworks(nn.Module):
    """A sequence of edge graph attention network (EGAT) layers"""
    ...
    def __init__(
        ...
        self.GAT_Layer = get_layer_type(gat_type)
        self.gats = nn.ModuleList()
        if output_dim is None:
            output_dim = input_dim_obj

        egat_kwargs_in = {
            "in_node_feats": input_dim_obj,
            "in_edge_feats": input_dim_pred,
            "out_node_feats": int(hidden_dim / num_heads),
            "out_edge_feats": int(hidden_dim / num_heads),
            "num_heads": num_heads,
        }
        ...
        egat_kwargs_out = {
            "in_node_feats": hidden_dim,
            "in_edge_feats": hidden_dim,
            "out_node_feats": output_dim,
            "out_edge_feats": output_dim,
            "num_heads": 1,
        }
        for i in range(self.num_layers):
            if i == 0:
                self.gats.append(self.GAT_Layer(**egat_kwargs_in))
            elif i >= self.num_layers - 1:
                self.gats.append(self.GAT_Layer(**egat_kwargs_out))
            else:
                self.gats.append(self.GAT_Layer(**egat_kwargs_mid))

    def forward(self, obj_vecs, pred_vecs, edges):
        graph = dgl.graph((edges[:, 0], edges[:, 1])).to(edges.device)
        for i in range(self.num_layers):
            gat = self.gats[i]
            obj_vecs, pred_vecs = gat(graph, obj_vecs, pred_vecs)
...
class EGATConv(nn.Module):
    ...
    def __init__(
        ...
        **kw_args,
        ...
        self.fc_edges = nn.Linear(
            in_edge_feats + 2 * in_node_feats, out_edge_feats * num_heads, bias=False
        )
    ...
    def forward(self, graph, nfeats, efeats):
        ...
        with graph.local_scope():
            ...
            num_nodes = nfeats.size(0)
            num_edges = efeats.size(0)
            ...
            new_node_feats = graph.ndata.pop("h")
            new_edge_feats = graph.edata.pop("f")

            new_node_feats = new_node_feats.reshape(
                num_nodes, -1
            )  # Reshape to (num_nodes, num_heads * out_node_feats)
            new_edge_feats = new_edge_feats.reshape(num_edges, -1)
