...
class VAE(nn.Module):
        ...
        if self.type_ == "v1_box":
            ...
            for id_name in id_names:
                latent_code = torch.load(
                    os.path.join(self.v1code_base, id_name, "sdf.pth"),
                    map_location="cpu",
                )[0]
                latent_code = latent_code.detach().numpy()
                self.code_dict[id_name] = latent_code[0]
            assert (
                distribution_before is not None
                and replace_latent is not None
                and with_changes is not None
            )
            self.vae = v1_full(
                vocab,
                embedding_dim=128,
                ...
                mlp_normalization="batch",
                gconv_num_layers=5,
                gconv_num_shared_layer=5,
                with_changes=with_changes,
                use_angles=with_angles,
                distribution_before=distribution_before,
                replace_latent=replace_latent,
                num_box_params=num_box_params,
                residual=residual,
                shape_input_dim=256 if deepsdf else 128,
            )
    ...
    def load_networks(self, exp, epoch, strict=True, restart_optim=False):
        if self.type_ == "v1_box":
            ...
            self.vae.load_state_dict(ckpt, strict=strict)
        elif self.type_ == "v2_box":
            self.vae_box.load_state_dict(
                torch.load(
                    os.path.join(exp, "checkpoint", "model_box_{}.pth".format(epoch)),
            ...
            self.vae_v2.load_state_dict(ckpt, strict=strict)
            print(
                colored(
                    "[*] v2_box successfully restored from: %s"
                    % os.path.join(exp, "checkpoint", "model{}.pth".format(epoch)),
                    "blue",
                )
            )
            ...
            if not restart_optim:
                ...
                self.vae_v2.scheduler = optim.lr_scheduler.LambdaLR(
                    self.vae_v2.optimizerFULL,
                    lr_lambda=self.vae_v2.lr_lambda,
            ...
            print(
                colored(
                    "[*] v2_shape successfully restored from: %s"
                    % os.path.join(exp, "checkpoint", "model{}.pth".format(epoch)),
                    "blue",
                )
            )
    ...
    def compute_statistics(self, exp, epoch, stats_dataloader, force=False):
        ...
        if self.type_ == "v1_box":
            ...
            if os.path.exists(stats_f) and not force:
                ...
                self.mean_est, self.cov_est = self.vae.collect_train_statistics(
                    stats_dataloader
                )
            ...
            if os.path.exists(stats_f) and not force:
                ...
                self.mean_est, self.cov_est = self.vae_v2.collect_train_statistics(
                    stats_dataloader
                )
                pickle.dump([self.mean_est, self.cov_est], open(stats_f, "wb"))

    def decoder_with_changes_boxes_and_shape(
        self,
        z_box,
        z_shape,
        objs,
        ...
        if self.type_ == "v1_full":
            boxes, feats, keep = self.vae.decoder_with_changes(
            ...
            boxes, sdfs, keep = self.vae_v2.decoder_with_changes(
                z_box,
                objs,
                triples,
                encoded_dec_text_feat,
    ...
    def decoder_with_changes_boxes(
        self,
        z,
        objs,
        triples,
    ...
    def decoder_with_changes_shape(
        self, z, objs, triples, attributes, missing_nodes, manipulated_nodes, atlas
    ):
        if self.type_ == "v1_full":
            return None, None

    def decoder_boxes_and_shape(
        self, z_box, z_shape, objs, triples, attributes, atlas=None
    ):
        angles = None
        if self.type_ == "v1_full":
            boxes, angles, feats = self.vae.decoder(z_box, objs, triples, attributes)
            points = (
                atlas.forward_inference_from_latent_space(feats, atlas.get_grid())
    ...
    def decoder_with_additions_boxes_and_shape(
        self,
        z_box,
        z_shape,
        objs,
        ...
        if self.type_ == "v1_full":
            outs, keep = self.vae.decoder_with_additions(
                z_box, objs, triples, attributes, missing_nodes, manipulated_nodes
            )
            ...
            boxes, sdfs, keep = self.vae_v2.decoder_with_additions(
                z_box,
                objs,
                triples,
                encoded_dec_text_feat,
    ...
    def decoder_with_additions_boxs(
        self,
        z,
        objs,
        triples,
    ...
    def encode_box_and_shape(
        ...
        if (
            ...
            with torch.no_grad():
                z, log_var = self.vae.encoder(
                    objs, triples, boxes, feats, attributes, angles
                )
    ...
    def sample_box_and_shape(
        ...
        if self.type_ == "v1_full":
            return self.vae.sample_3dfront(
                point_classes_idx,
                self.mean_est,
                self.cov_est,
                dec_objs,
        ...
        shapes = self.sample_shape(
            point_classes_idx, dec_objs, dec_triplets, attributes
        )
        return boxes, shapes
    ...
    def sample_box(
        ...
        if self.type_ == "v1_box" or self.type_ == "v2_box" or self.type_ == "v3_box":
            ...
            return self.vae.sample(
                self.mean_est, self.cov_est, dec_objs, dec_triplets, attributes
            )[0]
        elif self.type_ == "v2_full":
            return self.vae_v2.sample(
                self.mean_est,
    ...
    def save(self, exp, outf, epoch, counter=None):
        if self.type_ == "v1_box" or self.type_ == "v2_box" or self.type_ == "v3_box":
            ...
            torch.save(
                self.vae_v2.state_dict(epoch, counter),
                os.path.join(exp, outf, "model{}.pth".format(epoch)),
            )
