...
class RIODatasetSceneGraph(data.Dataset):
    def __init__(
        ...
        self.relationships = self.read_relationships(
            os.path.join(self.root, "relationships.txt")
        )
        ...
        if split == "train_scans":  # training set
            splits_fname = (
                "relationships_train_clean"
                if self.use_splits
                else "relationships_merged_train_clean"
            )
            ...
            self.floor_json_file = os.path.join(
                self.root, "floor_boxes_split_train.json"
            )
        else:  # validation set
            splits_fname = (
                "relationships_validation_clean"
                if self.use_splits
                else "relationships_merged_validation_clean"
            )
            self.rel_json_file = os.path.join(self.root, "{}.json".format(splits_fname))
        ...
        if self.use_rio27:
            self.vocab_rio27 = json.load(
                open(os.path.join(self.root, "classes_rio27.json"), "r")
            )
        ...
        self.mapping_full2rio27 = json.load(
            open(os.path.join(self.root, "mapping_full2rio27.json"), "r")
        )
        ...
        for pc in points_classes:
            if class_choice is not None:
                ...
                if not use_rio27:
                    ...
                    points_classes_idx.append(
                        int(self.vocab_rio27["rio27_name_to_idx"][pc])
                    )
            ...
            for scan in data["scans"]:
                ...
                for k, v in scan["objects"].items():
                    ...
                    try:
                        ...
                        boxes[int(k)]["param7"][6] = np.deg2rad(
                            boxes[int(k)]["param7"][6]
                        )
                        if self.use_canonical:
                            if "direction" in box_data[scan["scan"]][k].keys():
                                boxes[int(k)]["direction"] = box_data[scan["scan"]][k][
                                    "direction"
                                ]
                            else:
    ...
    def point_cloud_to_sdf(
        ...
        pcd.estimate_normals(
            search_param=o3d.geometry.KDTreeSearchParamHybrid(
                radius=normals_estimation_radius, max_nn=30
            )
        )
        ...
        sdf = o3d.pipelines.color_map.ComputeTSDFVolume(
            voxel_grid, pcd, sdf_truncation_distance
        )
    ...
    def norm_tensor(
        ...
        if rotation and params7 is not None:
            p = (
                torch.from_numpy(
                    get_rotation(-params7[-1], degree=False).astype("float32")
                )
                @ p.T
            ).T
        if scale and params7 is not None:
            ...
            if scale_func == "diag":
                ...
                p = (
                    (p - min_p) / norm2
                ) * 2.0 - 1.0  # between -1 and 1 in all directions
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(
            os.path.join(self.root_3rscan, scan_id_no_split, "semseg.v2.json")
        ):
            semseg_file = os.path.join(
                self.root_3rscan, scan_id_no_split, "semseg.v2.json"
            )
        elif os.path.exists(
            os.path.join(self.root_3rscan, scan_id_no_split, "semseg.json")
        ):
            semseg_file = os.path.join(
                self.root_3rscan, scan_id_no_split, "semseg.json"
            )
        else:
        ...
        if self.with_BERT:
            self.bert_feats_path = os.path.join(
                self.root_3rscan,
                scan_id.split("_")[0],
                "BERT_{}_{}.pkl".format(
                    "splits" if self.use_splits else "merged", scan_id.split("_")[1]
                ),
            )
            if self.crop_floor:
                self.bert_feats_path = os.path.join(
        ...
        if self.with_CLIP:
            self.clip_feats_path = os.path.join(
                self.root_3rscan,
                scan_id.split("_")[0],
                "CLIP_{}_{}.pkl".format(
                    "splits" if self.use_splits else "merged", scan_id.split("_")[1]
        ...
        if (
            self.with_feats and (not os.path.exists(feats_path) or self.recompute_feats)
        ) or self.use_points:
        ...
        for key in keys:
            ...
            if scene_instance_class in self.classes and (
                not self.use_rio27
                or self.mapping_full2rio27[scene_instance_class] != "-"
            ):
                if self.use_rio27:
                    scene_instance_class = self.mapping_full2rio27[scene_instance_class]
                    scene_class_id = int(
                        self.vocab_rio27["rio27_name_to_idx"][scene_instance_class]
                    )
                else:
                    scene_class_id = self.classes[
                        scene_instance_class
                    ]  # clasee id in the entire dataset ids
            ...
            if (
                (scene_class_id >= 0)
                and (scene_instance_id > 0)
                and (key in selected_instances)
            ):
                if self.use_canonical:
                ...
                if (
                    self.crop_floor
                    and key in self.floor_data[scan_id_no_split][split].keys()
                ):
                    bbox = self.floor_data[scan_id_no_split][split][key][
                        "params7"
                    ].copy()
                    bbox[6] = np.deg2rad(bbox[6])
                    direction = self.floor_data[scan_id_no_split][split][key][
                        "direction"
                    ]
        ...
        if self.use_points:
            masks = np.array(
                list(
                    map(
                        lambda l: instance2mask[l] if l in instance2mask.keys() else 0,
                        instances,
                    )
                ),
                dtype=np.int32,
            )
            num_pointsets = len(cat) + int(
                self.use_scene_rels
            )  # add zeros for the scene node
            ...
            for i in range(len(cat)):
                ...
                if (
                    self.crop_floor
                    and self.vocab["object_idx_to_name"][cat[i]].split("\n")[0]
                    == "floor"
                ):
                    filter_mask = (
                        (obj_pointset[:, 0] > min_box[0])
                        * (obj_pointset[:, 0] < max_box[0])
                        * (obj_pointset[:, 1] > min_box[1])
                        * (obj_pointset[:, 1] < max_box[1])
                    )

                    obj_pointset = obj_pointset[np.where(filter_mask > 0)[0], :]

                    print(
                        self.vocab["object_idx_to_name"][cat[i]].split("\n")[0],
                        len(obj_pointset),
                    )
                if len(obj_pointset) >= self.npoints:
                    choice = np.random.choice(
                        len(obj_pointset), self.npoints, replace=False
                    )
                else:
                    choice = np.arange(len(obj_pointset))
                    # use repetitions to fill some more points
                    choice2 = np.random.choice(
                        len(obj_pointset), self.npoints - choice.shape[0], replace=True
                    )
                ...
                if not self.vae_baseline:
                    obj_pointset = self.norm_tensor(
                        obj_pointset,
                        denormalize_box_params(tight_boxes[i]),
                        scale=True,
                        rotation=self.use_canonical,
                        scale_func=self.scale_func,
                    )
                else:
                    obj_pointset = self.norm_tensor(
                        obj_pointset,
                        np.asarray(tight_boxes[i]),
                        scale=True,
                        rotation=self.use_canonical,
                        scale_func=self.scale_func,
                    )
            ...
            if self.use_SDF:
                ...
                for obj_point in obj_points:
                    ...
                    obj_sdf = self.point_cloud_to_sdf(
                        point_cloud_data,
                        voxel_size,
                        normals_estimation_radius,
                        sdf_truncation_distance,
        ...
        for r in rel_json:  # create relationship triplets from data
            if r[0] in instance2mask.keys() and r[1] in instance2mask.keys():
                ...
                if subject >= 0 and object >= 0:
                    ...
                    words.append(
                        instance2label[r[0]] + " " + r[3] + " " + instance2label[r[1]]
                    )
            else:
                continue
        ...
        if (
            self.with_BERT
            and (not os.path.exists(self.bert_feats_path) or bert_feats_ins is None)
            ...
            with torch.no_grad():
                for i in range(num_cat - 1):
                    ...
                    feats_ins.append(
                        self.cond_model(self.get_key(self.classes, cat[i]))
                        .detach()
                        .cpu()
                        .numpy()
                    )  # 1*77*1280
                ...
                for i in range(len(words)):
                    feats_rel[words[i]] = (
                        self.cond_model(words[i]).detach().cpu().numpy()
                    )
            bert_feats_in = {}
        ...
        if (
            self.with_feats
            and (not os.path.exists(feats_path) or feats_in is None)
            and self.atlas is not None
        ):
            pf = (
                torch.from_numpy(np.array(list(obj_points.numpy()), dtype=np.float32))
                .float()
                .cuda()
                .transpose(1, 2)
            )
            ...
            if not self.eval:
                ...
                if output["manipulate"]["type"] == "addition":
                    ...
                    if suc:
                        ...
                        return -1
        # torchify
        output["encoder"]["objs"] = torch.from_numpy(
            np.array(output["encoder"]["objs"], dtype=np.int64)
        )  # this is changed
        output["encoder"]["triples"] = torch.from_numpy(
            np.array(output["encoder"]["triples"], dtype=np.int64)
        )
        output["encoder"]["boxes"] = torch.from_numpy(
            np.array(output["encoder"]["boxes"], dtype=np.float32)
        )
        if self.use_points:
            output["encoder"]["points"] = torch.from_numpy(
                np.array(output["encoder"]["points"], dtype=np.float32)
            )
        if self.with_feats:
            output["encoder"]["feats"] = torch.from_numpy(
                np.array(output["encoder"]["feats"], dtype=np.float32)
            )
        if self.with_BERT or self.with_CLIP:
            output["encoder"]["text_feats"] = torch.from_numpy(
                np.array(output["encoder"]["text_feats"], dtype=np.float32)
            )  # this is changed
            output["encoder"]["rel_feats"] = torch.from_numpy(
                np.array(output["encoder"]["rel_feats"], dtype=np.float32)
            )

        output["decoder"]["objs"] = torch.from_numpy(
            np.array(output["decoder"]["objs"], dtype=np.int64)
        )
        output["decoder"]["triples"] = torch.from_numpy(
            np.array(output["decoder"]["triples"], dtype=np.int64)
        )  # this is changed
        output["decoder"]["boxes"] = torch.from_numpy(
            np.array(output["decoder"]["boxes"], dtype=np.float32)
        )
        if self.use_points:
            output["decoder"]["points"] = torch.from_numpy(
                np.array(output["decoder"]["points"], dtype=np.float32)
            )
        if self.with_feats:
            output["decoder"]["feats"] = torch.from_numpy(
                np.array(output["decoder"]["feats"], dtype=np.float32)
            )
        if self.with_BERT or self.with_CLIP:
            output["decoder"]["text_feats"] = torch.from_numpy(
                np.array(output["decoder"]["text_feats"], dtype=np.float32)
            )
            output["decoder"]["rel_feats"] = torch.from_numpy(
                np.array(output["decoder"]["rel_feats"], dtype=np.float32)
            )  # this is changed
        ...
        while not did_change and trials < 1000:
            ...
            if interpretable:
                if (
                    ...
                    continue
                new_pred = interpretable_rels[
                    np.random.randint(1, len(interpretable_rels))
                ]
            else:
                new_pred = np.random.randint(1, 27)

            graph["words"][idx] = graph["words"][idx].replace(
                self.relationships[graph["triples"][idx][1]],
                self.relationships[new_pred],
            )
            if self.with_BERT:
