...
def fit_shapes_to_box_3dfront(obj, box, degrees=False):
    ...
    for i in [-1, 1]:
        for j in [-1, 1]:
            for k in [-1, 1]:
                box_points.append([l.item() / 2 * i, h.item() /2* j, w.item() / 2 * k])
...
def batch_torch_denormalize_box_params(box_params, file=None, scale=3,device = 'cpu'):
    ...
    """
    if file == None:
        mean = torch.from_numpy(np.array([1.3827214, 1.309359, 0.9488993, -0.12464812, 0.6188591, -0.54847]).reshape(1,-1)).to(device)
        std = torch.from_numpy(np.array([1.7797655, 1.657638, 0.8501885, 1.9160025, 2.0038228, 0.70099753]).reshape(1,-1)).to(device)
    else:
        stats = np.loadtxt(file)
        mean, std = torch.from_numpy(stats[0][:6].reshape(1,-1)).to(device), torch.from_numpy(stats[1][:6].reshape(1,-1)).to(device)
