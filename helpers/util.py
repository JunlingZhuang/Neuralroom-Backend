...
def fit_shapes_to_box_3dfront(obj, box, degrees=False):
    ...
    for i in [-1, 1]:
        for j in [-1, 1]:
            for k in [-1, 1]:
                box_points.append(
                    [l.item() / 2 * i, h.item() / 2 * j, w.item() / 2 * k]
                )
...
def fit_shapes_to_box_points(obj, box_points):
...
def get_textured_objects_v2(
    ...
    bbox_file = (
        "/home/ymxlzgy/code/graphto3d_v2/GT/3dfront/cat_jid_trainval.json"
        if datasize == "large"
        else "/home/ymxlzgy/code/graphto3d_v2/GT/3dfront/cat_jid_trainval_small.json"
    )
    colors = iter(colors)
    ...
    for j in range(0, boxes.shape[0]):
        ...
        furniture_id = get_closest_furniture_to_box(box_data[query_label], query_size)
        ...
        tr_mesh = trimesh.Trimesh(
            vertices=tr_mesh.vertices, faces=tr_mesh.faces, process=False
        )
        ...
        tr_mesh.export(
            os.path.join(
                mesh_dir,
                query_label + "_" + str(cat_ids[j]) + "_" + str(instance_id) + ".obj",
            )
        )
        instance_id += 1
        ...
        if render_boxes:
            ...
            trimesh_meshes.append(
                create_bbox_marker(box_points, tube_radius=0.006, color=color)
            )
...
def get_bbox(boxes, cat_ids, classes, colors, without_lamp=False):
    ...
    for j in range(0, boxes.shape[0]):
        ...
        box_points = params_to_8points_3dfront(boxes[j], degrees=True)
        trimesh_meshes.append(
            create_bbox_marker(box_points, tube_radius=0.02, color=next(colors))
        )
...
def trimeshes_to_pytorch3d(meshes):
    ...
    for mesh in meshes:
        ...
        vertex_colors = torch.tensor(mesh.visual.vertex_colors, dtype=torch.float32).to(
            device
        )  # [V, 4]
...
def pytorch3d_to_trimesh(pytorch3d_mesh):
    ...
    tri_mesh = trimesh.Trimesh(
        vertices=trimesh_verts, faces=trimesh_faces, process=False
    )
...
def get_sdfusion_models(
    ...
    os.makedirs(mesh_dir, exist_ok=True)
    sdfusion_model_path = (
        "/media/ymxlzgy/Data/Dataset/3D-FRONT/txt2shape_results_latest"
    )
    ...
    for j in range(0, boxes.shape[0]):
        ...
        color = next(colors)
        folder_path = (
            os.path.join(sdfusion_model_path, query_label)
            if mapping_full2simple == None
        ...
        if no_stool and query_label == "chair":
            ...
            model_path2 = random.choice(
                glob.glob(os.path.join(sdfusion_model_path, "stool", "*.ply"))
            )
...
def create_bbox_marker(corner_points, color=[0, 0, 255], tube_radius=0.002, sections=4):
    ...
    bbox_edge_list = []
    for edge in edges:
        bbox_edge = trimesh.creation.cylinder(
            radius=tube_radius,
            sections=sections,
            segment=[corner_points[edge[0]], corner_points[edge[1]]],
        )
...
def refineBoxes(boxes, objs, triples, relationships, vocab):
    for idx in range(len(boxes)):
        ...
        for t in triples:
            if idx == t[0] and relationships[t[1]] in [
                "supported by",
                "lying on",
                "standing on",
            ]:
                ...
                # fix adjusmets
                for t_ in triples:
                    if t_[2] == t[0] and relationships[t_[1]] in [
                        "supported by",
                        "lying on",
                        "standing on",
                    ]:
...
def get_rotation(z, degree=True):
    ...
    if degree:
        z = np.deg2rad(z)
    rot = np.array([[np.cos(z), -np.sin(z), 0], [np.sin(z), np.cos(z), 0], [0, 0, 1]])
    return rot
...
def batch_torch_denormalize_box_params(box_params, file=None, scale=3, device="cpu"):
    """Denormalize the box parameters utilizing the accumulated dataset statistics
    ...
    if file == None:
        ...
        stats = np.loadtxt(file)
        mean, std = torch.from_numpy(stats[0][:6].reshape(1, -1)).to(
            device
        ), torch.from_numpy(stats[1][:6].reshape(1, -1)).to(device)
...
def decode_latent_vector(z, model, dec_objs, dec_triples, box_file=None, box_data=None):
    model.eval()
    with torch.no_grad():
        samples = model.vae.decoder(z, dec_objs, dec_triples, attributes=None)

        boxes_pred, shapes_pred = samples[:-1], samples[-1]

        boxes_pred, angles_pred = boxes_pred

        angles_pred = -180 + (torch.argmax(angles_pred, dim=1, keepdim=True) + 1) * 15.0

        shapes_pred, _ = model.decode_g2sv1(
            dec_objs, shapes_pred, box_data, retrieval=True
        )

        boxes_pred_den = batch_torch_denormalize_box_params(boxes_pred, file=box_file)

    return boxes_pred_den, shapes_pred, angles_pred


def decode_latent_vector_box(
    z,
    model,
    dec_objs,
    dec_triples,
    ...
    with torch.no_grad():
        ...
        boxes_pred, angles_pred = samples

        angles_pred = -180 + (torch.argmax(angles_pred, dim=1, keepdim=True) + 1) * 15.0

        boxes_pred_den = batch_torch_denormalize_box_params(
            boxes_pred[:, :6], file=box_file, device=args["device"]
        )  # TODO: not sure works after using relative encoding

    return boxes_pred_den, angles_pred
